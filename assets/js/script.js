...
var skills = ["Public Relation", "Communication", "Editor"];
var index = 0;
var text = "";
...
function type() {
  ...
  if (isDeleting) {
    text = skill.substring(0, text.length - 1);
  } else {
    text = skill.substring(0, text.length + 1);
  }
  document.getElementById("typing-text").textContent = text;

  var speed = 200; // Kecepatan animasi

  if (isDeleting) {
    speed /= 2;
  }

  // Jika selesai menampilkan atau menghapus satu skill
  if (!isDeleting && text === skill) {
    isDeleting = true;
    speed = 500; // Waktu tunggu sebelum menghapus
  } else if (isDeleting && text === "") {
    isDeleting = false;
    index++; // Pindah ke skill selanjutnya
